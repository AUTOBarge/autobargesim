%%Control_demo
% Description
% Demo for the implementation of the PID controller (control class).
...
% Q(8,8): State [u v r x y psi delta n], R(2,2): Controls [delta_c n_c]
mpc_params = struct("Q",diag([0,0,0,10,50,0.1,0,0]),"R",diag([0.01,0.0]));
Flag_cont = 0;
psi_d=[pi/4];
%% Initialization
Vessel = modelClass(ship_dim);
SRSP = actuatorClass(ship_dim, prop_params, rud_params);
PIDobj=controlClass(pid_params,mpc_params, Flag_cont);
